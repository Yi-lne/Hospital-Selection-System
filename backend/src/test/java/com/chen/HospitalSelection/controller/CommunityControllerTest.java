...
public class CommunityControllerTest {
    ...
    @Test
    @DisplayName("测试回复评论 - 成功")
    @WithMockUser
    public void testReplyComment_Success() throws Exception {
        // Arrange
        CommentDTO dto = new CommentDTO();
        dto.setTopicId(1L);
        dto.setContent("这是一条回复");
        dto.setParentId(1L);  // 回复评论ID为1的评论

        when(communityService.addComment(anyLong(), any(CommentDTO.class))).thenReturn(2L);

        // Act & Assert
        mockMvc.perform(post("/community/comment/reply")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dto)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
    ...
    @Test
    @DisplayName("测试回复评论 - 失败（parentId无效）")
    @WithMockUser
    public void testReplyComment_InvalidParentId() throws Exception {
        // Arrange
        CommentDTO dto = new CommentDTO();
        dto.setTopicId(1L);
        dto.setContent("这是一条回复");
        dto.setParentId(0L);  // parentId为0不是有效的回复
        ...
        mockMvc.perform(post("/community/comment/reply")
                ...
                .andExpect(jsonPath("$.message").value(containsString("父评论ID")));
    }
    ...
    public void testDeleteComment_Success() throws Exception {
        // Arrange
        doNothing().when(communityService).deleteComment(anyLong(), anyLong());

        // Act & Assert
        mockMvc.perform(delete("/community/comment/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
                .andExpect(jsonPath("$.message").value("删除成功"));
    }
    ...
    public void testGetCommentList_Success() throws Exception {
        // Arrange
        com.chen.HospitalSelection.vo.PageResult<com.chen.HospitalSelection.vo.CommentVO> commentPageResult =
            new com.chen.HospitalSelection.vo.PageResult<>(1L, 1, 10, Arrays.asList());

        when(communityService.getCommentList(anyLong(), any(PageQueryDTO.class))).thenReturn(commentPageResult);

        // Act & Assert
        mockMvc.perform(get("/community/topic/1/comments")
                        .param("page", "1")
                        .param("pageSize", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
    ...
    public void testLikeComment_Success() throws Exception {
        // Arrange
        doNothing().when(communityService).likeComment(anyLong(), anyLong());

        // Act & Assert
        mockMvc.perform(post("/community/like/comment")
                        .param("commentId", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
    ...
    public void testUnlikeComment_Success() throws Exception {
        // Arrange
        doNothing().when(communityService).unlikeComment(anyLong(), anyLong());

        // Act & Assert
        mockMvc.perform(delete("/community/like/comment/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
    ...
    @Test
    @DisplayName("测试我的评论 - 成功")
    @WithMockUser
    public void testGetMyComments_Success() throws Exception {
        // Arrange
        com.chen.HospitalSelection.vo.PageResult<com.chen.HospitalSelection.vo.CommentVO> commentPageResult =
            new com.chen.HospitalSelection.vo.PageResult<>(1L, 1, 10, Arrays.asList());

        when(communityService.getMyComments(anyLong(), any(PageQueryDTO.class))).thenReturn(commentPageResult);

        // Act & Assert
        mockMvc.perform(get("/community/my/comments")
                        .param("page", "1")
                        .param("pageSize", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
