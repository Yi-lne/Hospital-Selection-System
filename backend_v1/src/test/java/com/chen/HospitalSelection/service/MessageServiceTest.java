...
public class MessageServiceTest {
    ...
    public void testGetConversations_Success() {
        // Arrange
        when(messageMapper.selectBySenderId(TEST_USER_ID)).thenReturn(Arrays.asList(testMessage));
        when(messageMapper.selectByReceiverId(TEST_USER_ID)).thenReturn(Collections.emptyList());
        when(userMapper.selectById(TEST_USER_ID)).thenReturn(testUser);
        when(userMapper.selectById(TEST_USER_ID_2)).thenReturn(testUser2);
        ...
        verify(messageMapper, times(1)).selectBySenderId(TEST_USER_ID);
    }
    ...
    public void testGetUnreadCount_Success() {
        ...
        // Act
        MessageUnreadVO result = messageService.getUnreadCount(TEST_USER_ID);

        // Assert
        assertNotNull(result);
        assertEquals(Integer.valueOf(10), result.getUnreadCount());
