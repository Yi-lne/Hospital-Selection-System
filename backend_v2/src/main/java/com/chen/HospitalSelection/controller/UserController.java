...
public class UserController {
    ...
    public Result<Map<String, Object>> register(@RequestBody @Valid UserRegisterDTO dto) {
        ...
        // 2. 生成包含角色信息的token（注册成功自动登录）
        String token = jwtUtil.generateToken(
            userVO.getId(),
            userVO.getPhone(),
            userVO.getRoles() != null ? userVO.getRoles() : new ArrayList<>()
        );
    ...
    /**
     * 发送验证码
     * 接口路径：POST /api/user/verification-code
     * 是否需要登录：否
     *
     * @param phone 手机号
     * @return 发送结果（开发环境会返回验证码）
     */
    ...
    public Result<Map<String, String>> sendVerificationCode(@RequestParam String phone) {
        ...
        if (code != null && !code.isEmpty()) {
            return Result.success(data, "验证码发送成功（开发测试模式：验证码为" + code + "）");
        } else {
