...
public class NotificationServiceImpl implements NotificationService {
    ...
    public int deleteAllReadNotifications(Long userId) {
        log.info("删除所有已读通知，用户ID：{}", userId);
        int deletedCount = notificationMapper.deleteAllReadByUserId(userId);
        log.info("所有已读通知删除成功，用户ID：{}，删除数量：{}", userId, deletedCount);
        return deletedCount;
    }
    ...
    public void createReportHandleNotification(Long reportId, Long reporterId, Integer result, String reason) {
        ...
        // 构建通知内容
        String content;
        String targetDesc = getReportTargetDescription(report);
        if (result == 1) {
            // 通过
            content = "你提交的关于" + targetDesc + "的举报已被管理员审核通过";
            if (reason != null && !reason.isEmpty()) {
                content += "。通过原因：" + reason;
            }
        } else if (result == 2) {
            // 驳回
            content = "你提交的关于" + targetDesc + "的举报已被管理员驳回";
            if (reason != null && !reason.isEmpty()) {
                content += "。驳回原因：" + reason;
            }
        } else {
