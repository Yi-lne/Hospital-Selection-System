...
const searchHospitals = async () => {
  ...
  try {
    loading.value = true
    const res = await adminApi.searchHospitals(searchKeyword.value, includeDeleted.value)
...
const manageDepartments = (hospital) => {
  // 保存滚动位置和分页信息
  sessionStorage.setItem('hospitalListScrollPosition', window.scrollY.toString())
  sessionStorage.setItem('hospitalListCurrentPage', currentPage.value.toString())
  sessionStorage.setItem('hospitalListPageSize', pageSize.value.toString())
  router.push({
...
onMounted(() => {
  // 恢复分页信息
  const savedPage = sessionStorage.getItem('hospitalListCurrentPage')
  const savedPageSize = sessionStorage.getItem('hospitalListPageSize')
  if (savedPage) {
  ...
  // 恢复滚动位置
  nextTick(() => {
    const savedPosition = sessionStorage.getItem('hospitalListScrollPosition')
    if (savedPosition) {
      setTimeout(() => {
    ...
    // 清除已保存的信息
    sessionStorage.removeItem('hospitalListScrollPosition')
    sessionStorage.removeItem('hospitalListCurrentPage')
    sessionStorage.removeItem('hospitalListPageSize')
  })
})
</script>
