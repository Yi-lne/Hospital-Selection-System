        ...
        <el-card class="topic-card">
          <div class="topic-header">
            <div class="author-info">
              ...
              <div>
                <h3>
                  {{ topicDetail.nickname || '匿名用户' }}
                  <span v-if="topicDetail.gender && topicDetail.gender !== 0" class="gender-icon">
                    <el-icon v-if="topicDetail.gender === 1" color="#409EFF"><Male /></el-icon>
                    <el-icon v-else-if="topicDetail.gender === 2" color="#F56C6C"><Female /></el-icon>
                  </span>
                </h3>
        ...
        <el-card class="comment-card">
          ...
          <div v-if="commentList.length > 0" class="comment-list">
            <div
              ...
              :ref="highlightedCommentId === comment.id ? 'highlightedComment' : null"
              class="comment-item"
                ...
                <div class="comment-content">
                  <div class="comment-header">
                    <span class="author-name">
                      ...
                      <span v-if="comment.gender && comment.gender !== 0" class="gender-icon">
                        <el-icon v-if="comment.gender === 1" color="#409EFF"><Male /></el-icon>
                        <el-icon v-else-if="comment.gender === 2" color="#F56C6C"><Female /></el-icon>
                      </span>
                    </span>
                  ...
                  >
                    ...
                    <div class="reply-content">
                      <div class="reply-header">
                        <span class="author-name">
                          {{ reply.nickname || '匿名用户' }}
                          <span v-if="reply.gender && reply.gender !== 0" class="gender-icon">
                            <el-icon v-if="reply.gender === 1" color="#409EFF"><Male /></el-icon>
                            <el-icon v-else-if="reply.gender === 2" color="#F56C6C"><Female /></el-icon>
                          </span>
                        </span>
    ...
    >
      ...
      >
        ...
        <el-form-item label="话题板块子类" prop="boardSub">
          <el-select
            v-model="editForm.boardSub"
            placeholder="请选择话题板块子类"
            style="width: 100%"
            :loading="editDiseaseBoardsLoading"
            :disabled="!editForm.boardType"
          >
            <el-option
              v-for="board in getEditBoardSubOptions(editForm.boardType)"
              :key="board"
              :label="board"
              :value="board"
            />
        ...
        >
          <el-select
            ...
            placeholder="请选择疾病分类"
            clearable
            :disabled="!editForm.boardSub"
            style="width: 100%"
          >
            <el-option
              v-for="item in filteredEditDiseaseOptions"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
...
import { Star, StarFilled, View, ChatDotRound, Delete, Check, Edit, Male, Female } from '@element-plus/icons-vue'
...
const highlightedCommentId = ref(Number(route.query.commentId) || null)
...
// 疾病板块选项（从数据库获取）
const editDiseaseBoards = ref([])
const editDiseaseBoardsLoading = ref(false)

// 保存完整的疾病数据（用于过滤）
const allEditDiseases = ref([])

// 加载疾病板块列表
const loadEditDiseaseBoards = async () => {
  try {
    editDiseaseBoardsLoading.value = true
    const res = await getDiseaseTree()
    allEditDiseases.value = res.data
    // 过滤出一级分类（parent_id = 0 或没有 parentId 的）
    editDiseaseBoards.value = res.data
    .filter(item => !item.parentId || item.parentId === 0)
    .map(item => item.diseaseName)
  } catch (error) {
    console.error('加载疾病板块失败:', error)
    editDiseaseBoards.value = ['心血管疾病', '内分泌疾病', '肿瘤科', '儿科疾病', '妇科疾病', '骨科疾病',
    '神经系统疾病']
  } finally {
    editDiseaseBoardsLoading.value = false
    }
}
...
// 话题板块大类对应的一级板块
const getEditBoardSubOptions = (boardType) => {
  if (!boardType) return []
  switch (boardType) {
    case 1: // 疾病板块 - 从数据库获取一级分类
      return editDiseaseBoards.value
    case 2: // 医院评价
...
// 加载疾病分类数据（用于非疾病板块的级联选择器）
const loadDiseases = async () => {
  try {
    const res = await getDiseaseTree()
    diseaseOptions.value = res.data.map(disease => ({
      id: disease.id,
      name: disease.diseaseName || disease.name,
      code: disease.diseaseCode,
      parentId: disease.parentId,
      level: disease.level,
      children: disease.children ? disease.children.map(child => ({
        id: child.id,
        name: child.diseaseName || child.name,
        code: child.diseaseCode,
...
// 根据话题板块子类过滤疾病分类选项（用于编辑）
const filteredEditDiseaseOptions = computed(() => {
  // 如果不是疾病板块，返回空数组
  if (editForm.boardType !== 1) {
    return []
  }
  // 安全检查：确保 allEditDiseases 已初始化
  if (!allEditDiseases.value || !Array.isArray(allEditDiseases.value)) {
    return []
  }
  // 如果选择了话题板块子类，过滤出对应的二级分类
  if (editForm.boardSub) {
    // 找到对应的一级疾病
    const level1Disease = allEditDiseases.value.find(
      item => item && item.diseaseName === editForm.boardSub
    )
    if (level1Disease && level1Disease.children && Array.isArray(level1Disease.children) && level1Disease.children.length > 0) {
      // 返回二级分类
...
// 监听话题板块大类变化，动态更新验证规则
watch(() => editForm.boardType, (newVal, oldVal) => {
  // 如果对话框没有打开，说明是初始化填充，不清空数据
  ...
  if (newVal === 1) {
    // 选择疾病板块，添加疾病分类必填验证
    editRules.diseaseCode = [
      { required: true, message: '请选择疾病分类', trigger: 'change' }
    ]
  } else {
    // 移除疾病分类验证
    delete editRules.diseaseCode
  }

  // 只清空话题板块子类，**不清空疾病分类**
  editForm.boardSub = ''
  // 清空级联选择器
  selectedEditDisease.value = []
...
// 监听话题板块子类变化，清空疾病分类
watch(() => editForm.boardSub, () => {
  // 如果对话框没有打开，说明是初始化填充，不清空疾病分类
  if (!editDialogVisible.value) {
    return
  }
  editForm.diseaseCode = ''
  selectedEditDisease.value = []
})
...
// 处理返回
const handleBack = () => {
  if (route.query.from === 'community' && route.query.tab) {
    router.push({
      path: '/community',
      query: {
        tab: route.query.tab,
        page: route.query.page || 1,
        scroll: route.query.scroll || 0
      }
    })
  } else {
    router.back()
...
const loadTopicDetail = async () => {
  try {
    ...
    if (highlightedCommentId.value) {
      // 检查是否是回复，如果是回复则先展开父评论
      await checkAndExpandParentComment()
...
// 检查并展开包含目标回复的父评论
const checkAndExpandParentComment = () => {
  if (!highlightedCommentId.value) return

  // 遍历评论列表，查找包含目标回复的父评论
  for (const comment of commentList.value) {
    if (comment.replies && comment.replies.length > 0) {
      const hasTargetReply = comment.replies.some(reply => reply.id === highlightedCommentId.value)
      if (hasTargetReply) {
...
const scrollToComment = () => {
  nextTick(() => {
    ...
    setTimeout(() => {
      // 同时查找评论和回复
      const commentElement = document.querySelector('.comment-item.highlighted')
      const replyElement = document.querySelector('.reply-item.highlighted')
      const targetElement = commentElement || replyElement
...
const showEditDialog = async () => {
  ...
  // 填充所有数据
  editForm.boardType = topicDetail.value.boardType
  editForm.boardSub = topicDetail.value.boardSub || ''
  editForm.diseaseCode = topicDetail.value.diseaseCode || ''
  editForm.title = topicDetail.value.title
  editForm.content = topicDetail.value.content

  // 如果是非疾病板块，填充级联选择器的值
  if (topicDetail.value.boardType !== 1 && topicDetail.value.diseaseCode) {
    selectedEditDisease.value = [topicDetail.value.diseaseCode]
  }
  ...
  await editFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const updateData = {
        ...
        boardSub: editForm.boardSub || '',
        diseaseCode: editForm.diseaseCode || ''
      }
    ...
    .comment-list {
      .comment-item {
        ...
        @keyframes highlight-pulse {
          ...
          50% {
            box-shadow: 0 0 20px 5px rgba(255, 169, 64, 0.2);
          }
        }
