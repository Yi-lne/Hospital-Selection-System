        ...
        <div class="section doctor-section">
          ...
          <div class="doctor-toolbar">
            ...
            <el-select
              ...
              style="width: 200px"
            >
              <el-option label="全部职称" :value="null" />
              <el-option
                ...
                <el-card class="doctor-card" shadow="hover" @click="handleDoctorClick(doctor)">
                  ...
                  <div class="doctor-body">
                    ...
                    <div v-if="doctor.specialty" class="info-row">
                      ...
                      <span class="value specialty">{{ doctor.specialty }}</span>
                    </div>
...
const filteredDoctorsByBackend = ref<any[]>([])  // 存储后端筛选返回的医生列表
const departments = ref<any[]>([])
const isHospitalCollected = ref(false)

// 筛选和分页
const filterDeptName = ref<string | null>(null)
const filterTitle = ref<string | null>(null)  // 职称筛选
...
const loadHospitalDetail = async () => {
  try {
    ...
    // 处理医生数据
    allDoctors.value = (doctorRes.data || []).map((item: any) => ({
      id: item.id,
...
const checkDoctorsCollectStatus = async () => {
  try {
    ...
    console.error('Failed to check doctors collect status:', error)
  }
}
...
// 切换医生收藏
const toggleCollectDoctor = async (doctor: any) => {
  try {
    ...
    console.error('Failed to toggle doctor collection:', error)
    ElMessage.error('操作失败')
  }
}
